...
class FileEditor < SingleUserResource
  ...
  def write_file(user,file,validation_proc)
    @file_lock.synchronize {
      ...
      FileUtils.move(@file_name, @file_name_bak) if @valid_file.call
  ...
  def revert(user)
    @file_lock.synchronize {
      ...
      raise 'You do not have this file checked out' unless checked_out_by?(user) 
      begin
        FileUtils.move(@file_name_bak, @file_name) unless @valid_file.call
      rescue =>ex
...
=begin
  def rollback(user)
    @file_lock.synchronize {
      raise 'You do not have this file checked out' unless checked_out_by?(user)
      FileUtils.move(@file_name_bak, @file_name)
      #call whenever
    }
  end
=end
  ...
  def clean_up_resource
    @file_lock.synchronize {
      FileUtils.move(@file_name_bak, @file_name) if (!@valid_file.call and @revertable.call)
    }
  end
...
=begin
  def to_backup(filename, move=false)
    ...
    if FileUtils.exists? filename
      new_filename = FileUtils.versioned_filename(filename)
      FileUtils.send(move ? :move : :copy, filename, new_filename)
    end
    return new_filename
  end
=end
